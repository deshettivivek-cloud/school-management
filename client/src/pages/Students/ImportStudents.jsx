...
const ImportStudents = () => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const response = await api.get('/import/students/template', { responseType: 'blob' });
  ...
  const handleCommit = async () => {
    ...
    setCommitting(true);
    try {
      const res = await api.post('/import/students/commit', {
      ...
      toast.error(error.response?.data?.message || 'Failed to commit student import.');
    } finally {
      setCommitting(false);
    }
  };
    ...
    <div className="page-container" style={{ padding: '2rem', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button className="btn btn-secondary" onClick={() => navigate('/students/directory')} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <ArrowLeft size={18} /> Back to Directory
          </button>
          <div>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <FileSpreadsheet size={24} style={{ color: 'var(--primary-600, #4f46e5)' }} /> Bulk Student Import
            </h1>
            <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '0.875rem' }}>
              Upload an Excel (.xlsx, .xls) or CSV file to import multiple student records in bulk.
            </p>
          </div>
        </div>
              ...
              <table className="table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)', textAlign: 'left', background: 'var(--bg-muted, #f9fafb)' }}>
                    ...
                    <th style={{ padding: '0.75rem 1rem', fontSize: '0.75rem' }}>Grade</th>
                    <th style={{ padding: '0.75rem 1rem', fontSize: '0.85rem' }}>Parent Name & Phone</th>
                    ...
                    displayedRows.map((r) => (
                      ...
                      >
                        ...
                        <td style={{ padding: '0.75rem 1rem', fontSize: '0.85rem' }}>
                          {r.data.grade || '-'}{r.data.section ? ` (${r.data.section})` : ''}
                        </td>

                        <td style={{ padding: '0.75rem 1rem', fontSize: '0.85rem' }}>
                          <div>{r.data.parent_name || '-'}</div>
                          <small style={{ color: 'var(--text-muted)' }}>{r.data.parent_phone || ''}</small>
                        </td>
